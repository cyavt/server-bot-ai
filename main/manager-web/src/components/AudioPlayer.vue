...
// Trả về đường dẫn biểu tượng tương ứng theo trạng thái âm lượng
...
const toggleMute = () => {
  ...
  // Nếu bỏ tắt tiếng và âm lượng là 0, thì khôi phục âm lượng mặc định
  if (!isMuted.value && volume.value === 0) {
    volume.value = 0.5
...
// Chức năng kéo thả thanh tiến trình
...
const updateSliderPosition = () => {
  nextTick(() => {
    ...
    // Tính toán vị trí tương đối với viewport
    const left = buttonRect.left + window.scrollX
    const top = buttonRect.top + window.scrollY

    // Định vị ở giữa nút, căn giữa theo chiều dọc
