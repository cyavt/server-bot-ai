...
'[{"key": "api_url","label": "Địa chỉ API","type": "string"},{"key": "voice","label": "Giọng","type": "string"},{"key": "output_dir","label": "Thư mục đầu ra","type": "string"},{"key": "authorization","label": "Ủy quyền","type": "string"},{"key": "appid","label": "ID ứng dụng","type": "string"},{"key": "access_token","label": "Mã truy cập","type": "string"},{"key": "cluster","label": "Cụm","type": "string"},{"key": "speed_ratio","label": "Tốc độ nói","type": "number"},{"key": "volume_ratio","label": "Âm lượng","type": "number"},{"key": "pitch_ratio","label": "Cao độ","type": "number"}]'
where `id` = 'SYSTEM_TTS_doubao';

-- Thêm nhà cung cấp ASR Alibaba Cloud
delete from `ai_model_provider` where `id` = 'SYSTEM_ASR_AliyunASR';
INSERT INTO `ai_model_provider` (`id`, `model_type`, `provider_code`, `name`, `fields`, `sort`, `creator`, `create_date`, `updater`, `update_date`) VALUES
('SYSTEM_ASR_AliyunASR', 'ASR', 'aliyun', 'Nhận dạng giọng nói Alibaba Cloud', '[{"key":"appkey","label":"AppKey ứng dụng","type":"string"},{"key":"token","label":"Token tạm thời","type":"string"},{"key":"access_key_id","label":"AccessKey ID","type":"string"},{"key":"access_key_secret","label":"AccessKey Secret","type":"string"},{"key":"output_dir","label":"Thư mục đầu ra","type":"string"}]', 5, 1, NOW(), 1, NOW());

-- Thêm cấu hình mô hình ASR Alibaba Cloud
delete from `ai_model_config` where `id` = 'ASR_AliyunASR';
INSERT INTO `ai_model_config` VALUES ('ASR_AliyunASR', 'ASR', 'AliyunASR', 'Nhận dạng giọng nói Alibaba Cloud', 0, 1, '{\"type\": \"aliyun\", \"appkey\": \"\", \"token\": \"\", \"access_key_id\": \"\", \"access_key_secret\": \"\", \"output_dir\": \"tmp/\"}', NULL, NULL, 6, NULL, NULL, NULL, NULL);

-- Cập nhật tài liệu hướng dẫn cấu hình mô hình ASR Alibaba Cloud
UPDATE `ai_model_config` SET 
`doc_link` = 'https://nls-portal.console.aliyun.com/',
`remark` = 'Hướng dẫn cấu hình ASR Alibaba Cloud:
...
5. Điền vào file cấu hình' WHERE `id` = 'ASR_AliyunASR';
...
(101, 'FIRMWARE_TYPE', 'Loại firmware', 'Từ điển loại firmware', 0, 1, NOW(), 1, NOW());
