...
change_role_function_desc = {
    ...
    "function": {
        "name": "change_role",
        "description": "Gọi khi người dùng muốn chuyển đổi vai trò/tính cách mô hình/tên trợ lý, các vai trò có thể chọn: [机车女友,英语老师,好奇小男孩]",
        "parameters": {
            "type": "object",
            "properties": {
                "role_name": {"type": "string", "description": "Tên vai trò cần chuyển đổi"},
                "role": {"type": "string", "description": "Nghề nghiệp của vai trò cần chuyển đổi"},
            },
...
def change_role(conn: "ConnectionHandler", role: str, role_name: str):
    ...
    if role not in prompts:
        return ActionResponse(
            action=Action.RESPONSE, result="Chuyển đổi vai trò thất bại", response="Vai trò không được hỗ trợ"
        )
    new_prompt = prompts[role].replace("{{assistant_name}}", role_name)
    conn.change_system_prompt(new_prompt)
    logger.bind(tag=TAG).info(f"Chuẩn bị chuyển đổi vai trò:{role},tên vai trò:{role_name}")
    res = f"Chuyển đổi vai trò thành công,tôi là {role}{role_name}"
    return ActionResponse(action=Action.RESPONSE, result="Chuyển đổi vai trò đã được xử lý", response=res)
