...
async def wait_for_exit() -> None:
    ...
    if sys.platform != "win32":  # Unix / macOS
        ...
        # Windows: await một future sẽ luôn pending,
        # Để KeyboardInterrupt nổi lên đến asyncio.run, nhằm loại bỏ vấn đề chặn thoát tiến trình do thread thông thường còn lại
        try:
...
async def monitor_stdin():
    ...
    while True:
        await ainput()  # Chờ đầu vào bất đồng bộ, tiêu thụ phím Enter
...
async def main():
    ...
    # Thứ tự ưu tiên auth_key: server.auth_key trong config > manager-api.secret > tự động tạo
    # auth_key dùng cho xác thực jwt, ví dụ xác thực jwt của giao diện phân tích thị giác, tạo token của giao diện ota và xác thực websocket
    ...
    logger.bind(tag=TAG).info(
        "Giao diện phân tích thị giác là\thttp://{}:{}/mcp/vision/explain",
        get_local_ip(),
        port,
    )
    ...
    logger.bind(tag=TAG).info(
        "Nếu muốn test websocket, vui lòng dùng trình duyệt Chrome mở file test_page.html trong thư mục test"
    )
    logger.bind(tag=TAG).info(
        "=============================================================\n"
    )
    ...
    try:
        ...
        print("Task bị hủy, đang dọn dẹp tài nguyên...")
    finally:
        # Dừng trình quản lý GC toàn cục
        await gc_manager.stop()

        # Hủy tất cả task (điểm sửa lỗi quan trọng)
        stdin_task.cancel()
        ws_task.cancel()
        if ota_task:
            ota_task.cancel()

        # Chờ task kết thúc (phải thêm timeout)
        await asyncio.wait(
            [stdin_task, ws_task, ota_task] if ota_task else [stdin_task, ws_task],
            timeout=3.0,
            return_when=asyncio.ALL_COMPLETED,
        )
        print("Server đã đóng, chương trình thoát.")
...
if __name__ == "__main__":
    try:
        ...
        print("Ngắt thủ công, chương trình kết thúc.")
