...
def decode_opus_from_file(input_file):
    """
    Giải mã dữ liệu Opus từ file p3, và trả về danh sách các gói dữ liệu Opus cùng tổng thời lượng.
    """
    opus_datas = []
    total_frames = 0
    sample_rate = 16000  # Tần số lấy mẫu file
    frame_duration_ms = 60  # Thời lượng frame
    ...
    with open(input_file, 'rb') as f:
        while True:
            ...
            # Giải nén thông tin header
            _, _, data_len = struct.unpack('>BBH', header)

            # Đọc dữ liệu Opus theo độ dài được chỉ định trong header
...
def decode_opus_from_bytes(input_bytes):
    """
    Giải mã dữ liệu Opus từ dữ liệu nhị phân p3, và trả về danh sách các gói dữ liệu Opus cùng tổng thời lượng.
    """
    import io
    opus_datas = []
    total_frames = 0
