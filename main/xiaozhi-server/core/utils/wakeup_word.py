        ...
        while True:
            try:
                ...
                if time.time() - self.start_time > self.timeout:
                    raise TimeoutError("Lấy khóa file timeout")
                time.sleep(0.1)
...
class WakeupWordsConfig:
    def __init__(self):
        ...
        self._cache_ttl = 1  # Thời gian hiệu lực cache (giây)
        self._lock_timeout = 5  # Thời gian timeout khóa file (giây)
    ...
    def _load_config(self) -> Dict:
        """Tải file cấu hình, sử dụng cơ chế cache"""
        current_time = time.time()

        # Nếu cache hợp lệ, trả về cache trực tiếp
        if (
    ...
    def _save_config(self, config: Dict):
        """Lưu cấu hình vào file, sử dụng khóa file để bảo vệ"""
        try:
    ...
    def update_wakeup_response(self, voice: str, file_path: str, text: str):
        """Cập nhật cấu hình phản hồi từ khóa"""
        try:
            # Lọc biểu tượng cảm xúc
            ...
            print(f"Cập nhật cấu hình phản hồi từ khóa thất bại: {e}")
            raise

    def generate_file_path(self, voice: str) -> str:
        """Tạo đường dẫn file audio, sử dụng hash của voice làm tên file"""
        try:
            # Tạo hash của voice
            voice_hash = hashlib.md5(voice.encode()).hexdigest()
