...
class AuthToken:
    def __init__(self, secret_key: str):
        self.secret_key = secret_key.encode()  # Chuyển đổi sang bytes
        # Tạo khóa mã hóa có độ dài cố định từ khóa bí mật (32 bytes cho AES-256)
        self.encryption_key = self._derive_key(32)

    def _derive_key(self, length: int) -> bytes:
        """Tạo khóa có độ dài cố định"""
        from cryptography.hazmat.primitives import hashes
        from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC

        # Sử dụng giá trị muối cố định (môi trường sản xuất thực tế nên sử dụng muối ngẫu nhiên)
        salt = b"fixed_salt_placeholder"  # Môi trường sản xuất nên đổi thành tạo ngẫu nhiên
        kdf = PBKDF2HMAC(
    ...
    def generate_token(self, device_id: str) -> str:
        ...
        # Tạo payload ngoài, chứa dữ liệu đã mã hóa
    ...
    def verify_token(self, token: str) -> Tuple[bool, Optional[str]]:
        ...
        :return: (Có hợp lệ không, ID thiết bị)
        """
        try:
            # Trước tiên xác thực JWT ngoài (chữ ký và thời gian hết hạn)
            ...
            # Kiểm tra lại thời gian hết hạn (xác thực kép)
