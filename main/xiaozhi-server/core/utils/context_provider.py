...
class ContextDataProvider:
    """Điền dữ liệu ngữ cảnh, chịu trách nhiệm lấy dữ liệu từ API được cấu hình"""
    ...
    def fetch_all(self, device_id: str) -> str:
        """Lấy tất cả dữ liệu ngữ cảnh được cấu hình"""
        ...
        for provider in context_providers:
            ...
            try:
                ...
                # Thêm device_id vào header yêu cầu
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(result, dict):
                        ...
                        self.logger.bind(tag=TAG).warning(f"API {url} trả về không phải là dictionary JSON")
                else:
                    self.logger.bind(tag=TAG).warning(f"API {url} yêu cầu thất bại: {response.status_code}")
