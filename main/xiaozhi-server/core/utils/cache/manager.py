...
class GlobalCacheManager:
    ...
    @property
    def logger(self):
        """Khởi tạo logger trễ để tránh import vòng"""
    ...
    def set(
        ...
        # Sử dụng TTL từ cấu hình hoặc TTL được truyền vào
        ...
        with self._locks[cache_name]:
            ...
            # Xử lý các chiến lược khác nhau
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                # Chiến lược LRU: nếu đã tồn tại thì di chuyển đến cuối
                if key in cache:
                    del cache[key]
                cache[key] = entry
                ...
                # Kiểm tra giới hạn kích thước
                if config.max_size and len(cache) > config.max_size:
                    # Chiến lược đơn giản: xóa ngẫu nhiên một mục
    ...
    def get(
        ...
        with self._locks[cache_name]:
            ...
            # Chiến lược LRU: di chuyển đến cuối
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                del cache[key]
                cache[key] = entry
    ...
    def clear(self, cache_type: CacheType, namespace: str = "") -> None:
        """Xóa toàn bộ cache được chỉ định"""
        cache_name = self._get_cache_name(cache_type, namespace)
    ...
    def invalidate_pattern(
        ...
        """Vô hiệu hóa các mục cache theo mẫu"""
        cache_name = self._get_cache_name(cache_type, namespace)
    ...
    def _maybe_cleanup(self, cache_name: str):
        ...
        if now - self._last_cleanup > config.cleanup_interval:
            ...
            if deleted > 0:
                ...
                self.logger.debug(f"Dọn dẹp cache {cache_name}: đã xóa {deleted} mục hết hạn")


# Tạo instance trình quản lý cache toàn cục
