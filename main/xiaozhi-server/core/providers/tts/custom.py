...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if isinstance(self.params, str):
            try:
                ...
                raise ValueError("Tham số cấu hình Custom TTS sai, không thể phân tích chuỗi thành đối tượng")
        elif not isinstance(self.params, dict):
            raise TypeError("Tham số cấu hình Custom TTS sai, vui lòng tham khảo hướng dẫn cấu hình")
    ...
    async def text_to_speak(self, text, output_file):
        ...
        if resp.status_code == 200:
            ...
            error_msg = f"Yêu cầu Custom TTS thất bại: {resp.status_code} - {resp.text}"
