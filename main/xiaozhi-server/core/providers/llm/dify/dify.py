...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        self.session_conversation_map = {}  # Lưu trữ ánh xạ session_id và conversation_id
            ...
            if self.mode == "chat-messages":
                for line in r.iter_lines():
                    if line.startswith(b"data: "):
                        ...
                        # Nếu không tìm thấy conversation_id, thì lấy conversation_id lần này
                        if not conversation_id:
                            conversation_id = event.get("conversation_id")
                            self.session_conversation_map[session_id] = (
                                conversation_id  # Cập nhật ánh xạ
                            )
                        # Lọc sự kiện message_replace, sự kiện này sẽ đẩy toàn bộ một lần
                    ...
                    if line.startswith(b"data: "):
                        ...
                        if event.get("event") == "workflow_finished":
                            if event["data"]["status"] == "succeeded":
                                ...
                                yield "【Phản hồi dịch vụ bất thường】"
            elif self.mode == "completion-messages":
                for line in r.iter_lines():
                    if line.startswith(b"data: "):
                        event = json.loads(line[6:])
    ...
    def response_with_functions(self, session_id, dialogue, functions=None):
        if len(dialogue) == 2 and functions is not None and len(functions) > 0:
            # Lần đầu gọi llm, lấy tin nhắn người dùng cuối cùng, thêm lời nhắc tool
        ...
        # Nếu phần cuối cùng có role="tool", thêm vào user
