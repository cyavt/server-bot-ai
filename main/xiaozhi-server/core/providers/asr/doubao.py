...
def parse_response(res):
    ...
    reserved （8bits) Trường dành riêng
    header_extensions Tiêu đề mở rộng (kích thước bằng 8 * 4 * (header_size - 1) )
    payload Tương tự như body yêu cầu http
    """
...
class ASRProvider(ASRProviderBase):
        ...
        try:
            ...
            async with websockets.connect(
                ...
                if (
                    ...
                    and result["payload_msg"]["code"] != 1013  # Bỏ qua lỗi không có giọng nói hợp lệ
                ):
                ...
                if (
                    ...
                    # Không có giọng nói hợp lệ, trả về chuỗi rỗng
                    return ""
                else:
                    logger.bind(tag=TAG).error(f"ASR error: {result}")
                    return None
    ...
    async def speech_to_text(
        ...
        try:
            ...
            # Sử dụng trực tiếp dữ liệu PCM
            # Tính toán kích thước phân đoạn (đơn kênh, 16bit, tốc độ lấy mẫu 16kHz)
