...
class OTAHandler(BaseHandler):
    ...
    def _refresh_bin_cache_if_needed(self):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Làm mới cache firmware thất bại: {e}")
    ...
    def generate_password_signature(self, content: str, secret_key: str) -> str:
        ...
        Args:
            content: Nội dung chữ ký (clientId + '|' + username)
    ...
    def _get_websocket_url(self, local_ip: str, port: int) -> str:
        ...
        # Xử lý None, null, hoặc chuỗi rỗng
        if websocket_config and isinstance(websocket_config, str) and "你的" not in websocket_config:
    ...
    async def handle_post(self, request):
        ...
        - cố gắng xác định model thiết bị và phiên bản firmware hiện tại (ưu tiên headers, fallback về body)
        - kiểm tra data/bin để tìm firmware mới hơn cho model đó
        - nếu tìm thấy firmware mới hơn, đặt firmware.url thành endpoint tải xuống
        """
        try:
            ...
            if device_id:
                self.logger.bind(tag=TAG).info(f"ID thiết bị yêu cầu OTA: {device_id}")
            else:
                raise Exception("ID thiết bị yêu cầu OTA trống")

            client_id = request.headers.get("client-id", "")
            if client_id:
                self.logger.bind(tag=TAG).info(f"ClientID yêu cầu OTA: {client_id}")
            else:
                raise Exception("ClientID yêu cầu OTA trống")
            ...
            if mqtt_gateway_endpoint:  # Nếu đã cấu hình chuỗi không rỗng
                # Thử lấy model thiết bị từ dữ liệu yêu cầu (đã phân tích ở trên)
                try:
                ...
                try:
                    ...
                    self.logger.bind(tag=TAG).error(f"Tạo tên người dùng thất bại: {e}")
                    username = ""
                ...
                if signature_key:
                    ...
                    if not password:
                        password = ""  # Nếu tạo chữ ký thất bại thì để trống, để thiết bị quyết định có cho phép không mật khẩu
                else:
                    self.logger.bind(tag=TAG).warning("Thiếu khóa chữ ký MQTT, mật khẩu để trống")

                # Xây dựng cấu hình MQTT (sử dụng trực tiếp chuỗi mqtt_gateway)
                return_json["mqtt"] = {
                ...
                self.logger.bind(tag=TAG).info(f"Gửi cấu hình gateway MQTT cho thiết bị {device_id}")

            else:  # Chưa cấu hình mqtt_gateway, gửi WebSocket
                # Nếu đã bật xác thực, thì thực hiện kiểm tra xác thực
                token = ""
                if self.auth_enable:
                    if self.allowed_devices:
                        ...
                        token = self.auth.generate_token(client_id, device_id)
                # NOTE: sử dụng websocket_port ở đây
                return_json["websocket"] = {
                    "url": self._get_websocket_url(local_ip, websocket_port),
                    "token": token,
                }
                self.logger.bind(tag=TAG).info(
                    f"Chưa cấu hình gateway MQTT, gửi cấu hình WebSocket cho thiết bị {device_id}"
                )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Tìm firmware cho model {device_model}, tìm thấy {len(candidates)} ứng viên"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Gửi firmware {chosen_version} cho thiết bị {device_id} [Nếu tiền tố địa chỉ sai, vui lòng kiểm tra server.vision_explain trong file cấu hình]-> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
                        f"Thiết bị {device_id} đã có firmware mới nhất: {device_version}"
                    )
            ...
            self.logger.bind(tag=TAG).error(f"Xử lý OTA POST bất thường: {e}")
    ...
    async def handle_get(self, request):
        ...
        try:
            server_config = self.config["server"]
            local_ip = get_local_ip()
            # sử dụng cổng websocket cho URL websocket
            websocket_port = int(server_config.get("port", 8000))
            websocket_url = self._get_websocket_url(local_ip, websocket_port)
            message = f"Giao diện OTA hoạt động bình thường, địa chỉ websocket gửi đến thiết bị là: {websocket_url}"
            response = web.Response(text=message, content_type="text/plain")
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"Yêu cầu OTA GET bất thường: {e}")
            response = web.Response(text="Giao diện OTA bất thường", content_type="text/plain")
        finally:
            self._add_cors_headers(response)
            return response
    ...
    async def handle_download(self, request):
        """
        Giao diện tải xuống firmware
        URL: /xiaozhi/ota/download/{filename}
        - Chỉ cho phép tải xuống file .bin trong thư mục data/bin
        - filename phải là basename và khớp với pattern an toàn
        """
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Tải xuống firmware bất thường: {e}")
