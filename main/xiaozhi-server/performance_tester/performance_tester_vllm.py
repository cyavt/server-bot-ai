...
description = "Kiểm tra hiệu suất mô hình nhận dạng thị giác"
...
class AsyncVisionPerformanceTester:
    def __init__(self):
        ...
        self.test_questions = [
            ...
            "Vui lòng mô tả chi tiết nội dung của bức ảnh này",
        ]
    ...
    async def _test_vllm(self, vllm_name: str, config: Dict) -> Dict:
        """Kiểm tra hiệu suất một mô hình thị giác lớn bất đồng bộ"""
        try:
            # Kiểm tra cấu hình API key
            if "api_key" in config and any(
                x in config["api_key"] for x in ["你的", "placeholder", "sk-xxx"]
            ):
                print(f"⏭️  VLLM {vllm_name} chưa cấu hình api_key, đã bỏ qua")
            ...
            # Thực thi tất cả các kiểm tra đồng thời
            ...
            print(f"⚠️ VLLM {vllm_name} kiểm tra thất bại: {str(e)}")
            return {"name": vllm_name, "type": "vllm", "errors": 1}

    async def _test_single_vision(
        self, vllm_name: str, vllm, question: str, image: str
    ) -> Dict:
        """Kiểm tra hiệu suất một câu hỏi thị giác đơn lẻ"""
        try:
            print(f"📝 {vllm_name} bắt đầu kiểm tra: {question[:20]}...")
            ...
            print(f"⚠️ {vllm_name} kiểm tra thất bại: {str(e)}")
            return None
    ...
    def _print_results(self):
        ...
        if vllm_table:
            print("\nBảng xếp hạng hiệu suất mô hình thị giác lớn:\n")
            print(
                tabulate(
                    vllm_table,
            ...
            print("\n⚠️ Không có mô hình thị giác lớn khả dụng để kiểm tra.")

    async def run(self):
        """Thực thi kiểm tra bất đồng bộ toàn bộ"""
        print("🔍 Bắt đầu lọc các mô hình thị giác lớn khả dụng...")

        if not self.test_images:
            print(f"\n⚠️  Không có file ảnh trong đường dẫn {self.image_root}, không thể thực hiện kiểm tra")
            return
        ...
        if self.config.get("VLLM") is not None:
            for vllm_name, config in self.config.get("VLLM", {}).items():
                if "api_key" in config and any(
                    ...
                    print(f"⏭️  VLLM {vllm_name} chưa cấu hình api_key, đã bỏ qua")
                    continue
                print(f"🖼️ Thêm task kiểm tra VLLM: {vllm_name}")
                all_tasks.append(self._test_vllm(vllm_name, config))

        print(f"\n✅ Tìm thấy {len(all_tasks)} mô hình thị giác lớn khả dụng")
        print(f"✅ Sử dụng {len(self.test_images)} ảnh kiểm tra")
        print(f"✅ Sử dụng {len(self.test_questions)} câu hỏi kiểm tra")
        print("\n⏳ Bắt đầu kiểm tra đồng thời tất cả các mô hình...\n")
