...
graph TB
    ...
    ESP32 -->|HTTP<br/>Port: 8003| PythonServer
...
| **ESP32** | Python Server | HTTP | 8003 | Vision API, OTA updates |
...
| **Browser** | Python Server | HTTP | 8003 | Test page, OTA interface |
...
1. **ESP32** kết nối trực tiếp với **Python Server** qua WebSocket/HTTP
2. **Python Server** lấy config từ **Manager-API** qua HTTP (không kết nối trực tiếp MySQL)
3. **Manager-API** kết nối trực tiếp với **MySQL** và **Redis**
4. **Manager-Web** kết nối với **Manager-API** qua HTTP proxy
5. **Python Server** phụ thuộc vào MySQL/Redis (depends_on) nhưng không kết nối trực tiếp, chỉ để đảm bảo services sẵn sàng
